...
const handler = async (req, res) => {
  const { User, UserRole, UserStakeholder, Stakeholder, Form, UserForm, FormType } =
  ...
  if (role == 1) {
    list = await UserForm.findAll({
      ...
      attributes: ['HeadId', 'DeanId', 'CoordinatorId', 'StakeholderId', 'GraderId', 'Value']
  }).map((t) => {
    ...
    list = await UserForm.findAll({
      include: {
          ...
          where: {IsApproved: false, DepartmentId: userData.DepartmentId},
      ...
      return{
          ...
          FormName: t.dataValues.Form.dataValues.Name,
          FormType:t.dataValues.Form.FormType.dataValues.Name,
          InsertedDate:t.dataValues.Form.dataValues.InsertedDate
      }
    });
  } else if (role == 4) {
  } else if (role == 5) {
